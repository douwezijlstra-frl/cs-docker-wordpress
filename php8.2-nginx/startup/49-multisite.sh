# check if wordpress is installed, else continue
if [ -f /var/www/html/wordpress/wp-config.php ]; then

    # if wp config get MULTISITE returns 'true', then multisite is enabled
    if [[ $(sudo -u www-data wp --path=/var/www/html/wordpress config get MULTISITE) == "1" ]]; then
        
    # if wp config get SUBDOMAIN_INSTALL returns 'true', then subdomain is enabled
    if [[ $(sudo -u www-data wp --path=/var/www/html/wordpress config get SUBDOMAIN_INSTALL) == "1" ]]; then
